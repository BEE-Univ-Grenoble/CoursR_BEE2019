...
*str_length* : connaître le nombre de caractères dans un vecteur de chaînes                          
...
```{r echo=TRUE}
vec = c("chat","potté !")
str_sub(vec,2,4) # connaitre les caractères entre la 2ème et la 5ème position
```
...
Autres fonctions : </br> </br>
 - *str_to_lower* : mettre en minuscules les éléments d’une chaîne de caractères </br> </br>
...
```{r echo=TRUE}
grepl("^cha*t", c("achat", "chaat", "chti")) # équivalent string_detect
```

##

*regexpr* renvoie un vecteur (d'integers) de même longueur que la chaîne de caracètres, donnant la position de départ de la première correspondance ou -1 s'il n'y en a pas. Il renvoie aussi l'attribut "match.length" (integers) donnant la longueur du texte trouvé (ou -1 si pas de correspondance).
