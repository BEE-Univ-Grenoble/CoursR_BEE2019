...
Il permet d'eviter les boucles en iterant une fonction sur chaque objet de la liste.
...
Celles qui retournent des dataframes crees par jointure de lignes ou de colonnes 
  ...
  Il existe d'autres fonctions, qui prennent ET retournent des listes. Ce sont les lmap()
  Elles peuvent retourner des listes plus importantes que celles en argument.
  Voir retourer des listes vides.
...
`map_at(.x, .at, .f)`
  ...
  .x = objet
  .at = vecteur de nom au format caracteres,ou un vecteur de numeriques positifs, ou de numeriques negatifs
  .f = fonction appliquee
...
```
##
deux methodes pour faire  la moyenne de chaque liste
```{r}
...
type (NULL) incorrect pour la variable 'y'
  ...
  Soucis car le troisieme objet de la liste est nul. 
  Pour faire apparaitre les deux premiers qui sont bons on utilise safely()
